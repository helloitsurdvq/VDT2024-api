...
const { getAllTrainees, getOneTrainee, saveTrainee, updateTrainee, deleteTrainee } = require('../controllers/traineeController');
const { expect } = chai;
...
describe('Trainee Controller', () => {
  ...
  describe('/GET getAllTrainees', () => {
    it('should return all trainees with a 200 status code', async () => {
      const mockTrainees = [
        { _id: "6646593c421113400dc71794", name: 'test1', email: 'tes1@gmail.com', gender: 'Nam', school: 'Học viện Công Nghệ Bưu Chính Viễn Thông' },
        { _id: "6646e29d9ab7288d89c15516", name: 'test2', email: 'test2@soict.hust.edu.vn', gender: 'Nam', school: 'HUST' }
      ];
  ...
  describe('/DELETE/:id deleteTrainee', () => {
    it('should delete a trainee and return a 200 status code', async () => {
      ...
      expect(res.status.calledWith(200)).to.be.true;
      expect(res.send.calledWith({ message: "Trainee deleted successfully" })).to.be.true;
    });
